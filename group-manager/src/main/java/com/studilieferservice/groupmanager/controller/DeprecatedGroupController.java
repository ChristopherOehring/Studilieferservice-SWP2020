...
import com.studilieferservice.groupmanager.controller.bodys.PutUserBody;
...
@RestController
public class DeprecatedGroupController {
     ...
     */
    @Autowired
    public DeprecatedGroupController(final GroupService groupService, UserService userService) {
    ...
    public Gruppe createGroup(@PathVariable("name") String name, @RequestBody PutUserBody body) {
        ...
        if (userService.findById(owner.getEmail()).isEmpty()) {
    ...
    public String addUser(@RequestBody PutUserBody body){
        ...
        if (body.getUser().isValidEmailAddress(body.getUserID())) {
            ...
            g.addMember(u);
            if (userService.findById(body.getUserID()).isEmpty()) {
                userService.save(u);
